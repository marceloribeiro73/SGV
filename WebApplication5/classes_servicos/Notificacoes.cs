...
namespace WebApplication5
{
    public class Notificacoes
    {
        ...
        {
            string strCmd = string.Format("SELECT COUNT(V.CPF) AS 'PENDENCIAS' FROM VOLUNTARIO V, TERMO_ADSAO TA WHERE V.CPF =  TA.VOUNTARIO AND  TA.PATH = NULL OR V.CPF   NOT IN (SELECT V.CPF FROM VOLUNTARIO V, TERMO_ADSAO TA WHERE V.CPF = TA.VOLUNTARIO)");
